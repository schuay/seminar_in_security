...
proof that at most one slot was chosen. The input of each member $i$ in round $j$ is
a tuple $X_{ij} = (m_{ij}, g_{ij})$, where $m_{ij}$ is the message to be transmitted, and
$g_{ij}$ identifies the group of the intended recipient. $i$ sets $X_{it_i}$ to $(m_i, g_i)$,
and $X_{ij} = (nil, nil)$ for all other rounds $j \neq t_i$.

In each round $j$, the subgroup then performs the multiparty sum protocol to compute
$\sum_i X_{ij}$. Pedersen commitments are used to verify a) partitions of individual sum
elements $X_{ij}$, b) partial sums, and c) the final result. If no collision takes place,
$\sum_i X_{ij} = (m_i, g_i)$ for the transmitting member $i$ in round $j$, since all
other inputs $X_{ij} = (nil, nil)$. Each member of the subgroup finally broadcasts the
